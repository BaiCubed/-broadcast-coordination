...
def allocate_counts(weights: dict[str, float], total: int) -> dict[str, int]:
    ...
    if total < len(normalized):
        raise ValueError("fleet size is smaller than the number of component datasets")
...
def _max_unique_allocation(
    ...
    raise ValueError("not enough real sources to build a source-unique fleet")
...
class FleetGenerator:
    ...
    def build_fleet(
        ...
        if sampling_mode == "unique" and duplicate_count:
            raise RuntimeError("the source-unique fleet repeats a real source")
        return devices, {
    ...
    def scenario_fleet(
        ...
        if partition_name == "train":
            ...
            if not 0 <= seed_index < TEST_SEED_COUNT:
                raise ValueError("test seed index must be within 0..29")
            seed = test_start + seed_index
            weights, regime = shifted_weights(scenario["weights"], seed_index)
        else:
            raise ValueError("the test partition requires a seed index")
